...
import { IAny, SharedConfig, useContext } from "reblendjs";
import md5, { rand } from "./md5";
import { API_VERSION, BASE } from "./RestEndpoints";
import axios, { AxiosRequestConfig, AxiosResponse } from "axios";
import { authTokenContext } from "./contexts";
import IResStruct from "../interfaces/IResStruct";
...
export class Fetcher {
  ...
  async fetch<ResponseDataType, RequestDataType = IAny>(
    options: string | AxiosRequestConfig<RequestDataType>,
    returnType = Fetcher.RETURN_JSON_OBJECT
  ): Promise<IResStruct<ResponseDataType>> {
    let url;
